...
async function onConfirmYes() {
  ...
  if (!result.eligible) {
    ...
    document.getElementById("examListArea").innerHTML = `
      ...
      <p style="font-size:13px; color:var(--text-sub); margin-bottom:6px;">아직 이수하지 못한 프로그램:</p>
      <ul style="font-size:14px;">
        ${(result.incomplete_programs || []).map((name) => `<li>${name}</li>`).join("")}
      </ul>`;
