...
async function loadResultsForSelectedExam() {
  ...
  const [{ data: students }, { data: enrollments }] = await Promise.all([
    fetchAllRows("students", "student_no, name"),
    fetchAllRows("enrollments", "student_id, completed_hours, students(student_no)"),
  ]);

  const hoursByStudentNo = {};
  (enrollments || []).forEach((e) => {
    const no = e.students?.student_no;
    if (!no) return;
    hoursByStudentNo[no] = (hoursByStudentNo[no] || 0) + Number(e.completed_hours || 0);
  });

  const target = (students || []).filter((s) => (hoursByStudentNo[s.student_no] || 0) >= REQUIRED_TOTAL_HOURS);
