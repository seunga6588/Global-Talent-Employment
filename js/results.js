...
async function loadExamOptions() {
  const { data } = await supabaseClient.from("exams").select("id, title, program_id").order("created_at", { ascending: false });
  examsForSelect = data || [];
...
async function loadResultsForSelectedExam() {
  ...
  if (!examId) return;
  const exam = examsForSelect.find((e) => e.id === examId);
  ...
  // 응시대상자 = 해당 프로그램에 등록(enrollment)된 전체 학생
  let target = [];
  if (exam?.program_id) {
    ...
    target = (enrolled || []).map((e) => e.students).filter(Boolean);
  }
