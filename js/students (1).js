...
async function uploadExcel() {
  ...
  // "이수시간_프로그램명" 형태의 컬럼을 프로그램 ID로 매칭하기 위한 맵
  const programNameToId = {};
  programsListCache.forEach((p) => (programNameToId[`이수시간_${p.name}`] = p.id));

  const payloads = [];
  const enrollmentUpdates = []; // { student_no, program_id, completed_hours }
  ...
  rows.forEach((row) => {
    ...
    Object.entries(row).forEach(([rawHeader, value]) => {
      ...
      if (programNameToId[header]) {
        // 프로그램별 이수시간 컬럼
        if (value !== "" && value !== undefined) {
          enrollmentUpdates.push({ program_id: programNameToId[header], completed_hours: Number(value) || 0, __row: row });
        }
        return;
      }
  ...
  // 2) 프로그램별 이수시간 컬럼이 있었다면 enrollments 테이블도 함께 갱신
  if (enrollmentUpdates.length) {
    ...
    const rowsToUpsert = enrollmentUpdates
      .map((u) => ({
        student_id: noToId[u.__row["학번"]],
        program_id: u.program_id,
        completed_hours: u.completed_hours,
      }))
      .filter((u) => u.student_id);

    if (rowsToUpsert.length) {
      await supabaseClient.from("enrollments").upsert(rowsToUpsert, { onConflict: "student_id,program_id" });
    }
  }
...
function downloadTemplate() {
  ...
  // 프로그램별 이수시간 컬럼을 현재 등록된 프로그램 기준으로 자동 추가
