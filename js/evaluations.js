...
let deptQuotaCache = {};      // { 학과: 정원 }
let gradedCountByDept = {};   // { 학과: 현재 등급 부여된 인원 수 }
...
async function loadEvalData() {
  const [{ data: students }, { data: enrollments }, { data: programs }, { data: attempts }, { data: evaluations }, { data: quotas }] = await Promise.all([
  ...
  deptQuotaCache = {};
  (quotas || []).forEach((q) => (deptQuotaCache[q.department] = q.quota));
  ...
  (evaluations || []).forEach((ev) => {
    ...
    gradedCountByDept[s.department] = (gradedCountByDept[s.department] || 0) + 1;
  });
...
function openEvalModal(studentId) {
  ...
  const quota = deptQuotaCache[s.department] || 0;
  const currentCount = gradedCountByDept[s.department] || 0;
  const quotaInfo = document.getElementById("evalQuotaInfo");
  if (quotaInfo) {
    quotaInfo.textContent = quota > 0 ? `${s.department} 학과 우수학생 선정 현황: ${currentCount} / ${quota}명` : `${s.department} 학과 정원이 아직 설정되지 않았습니다.`;
  }
...
async function saveEvaluation() {
  ...
  const quota = deptQuotaCache[dept] || 0;

  if (quota > 0 && dept) {
    const previousGrade = student.evaluation?.grade || null;
    const currentCount = gradedCountByDept[dept] || 0;
    // 이 학생이 기존에 등급이 있었다면 빼고, 새로 등급을 받는다면 더해서 예상 인원을 계산
    const projectedCount = currentCount - (previousGrade ? 1 : 0) + (grade ? 1 : 0);

    if (grade && projectedCount > quota) {
      alert(
        `⚠️ ${dept} 학과의 우수학생 선정 정원(${quota}명)을 초과합니다.\n` +
        `현재 선정 인원: ${currentCount}명\n\n` +
        `이 학생에게 등급을 부여하려면 먼저 "학과별 인원 설정"에서 정원을 늘리거나,\n` +
        `다른 학생의 등급을 조정해주세요.`
      );
      return;
    }
