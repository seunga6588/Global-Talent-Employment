...
(async function init() {
  ...
  if (!auth) return;
  renderLayout("exams", auth.admin.name, auth.admin.department);
    ...
    .map((e) => {
      ...
      return `<tr>
        ...
        <td class="text-nowrap">
          ...
          <button class="btn btn-outline-navy btn-sm" onclick="openEditExam('${e.id}')">수정</button>
          ...
          <button class="btn btn-outline-secondary btn-sm" onclick="deleteExam('${e.id}')">삭제</button>
        </td>
      </tr>`;
    })
    .join("");
...
async function saveExam() {
  ...
  let error;
  if (id) {
    ({ error } = await supabaseClient.from("exams").update(payload).eq("id", id));
  } else {
    ({ error } = await supabaseClient.from("exams").insert(payload));
  }
...
function openAddExam() {
  document.getElementById("examModalTitle").textContent = "시험 추가";
  const form = document.getElementById("examForm");
  form.reset();
...
function openEditExam(id) {
  ...
  form.elements["retake_allowed"].value = String(e.retake_allowed);
  new bootstrap.Modal(document.getElementById("examModal")).show();
}

async function deleteExam(id) {
  if (!confirm("이 시험을 삭제하시겠습니까? 등록된 문제와 학생 응시 결과가 모두 함께 삭제됩니다.")) return;
