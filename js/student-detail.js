...
(async function init() {
  ...
  if (!auth) return;
  renderLayout("students", auth.admin.name, auth.admin.department);
  ...
  document.getElementById("saveEmploymentBtn").addEventListener("click", saveEmployment);
})();
...
async function loadStudentDetail(studentNo) {
  ...
  document.getElementById("employmentBox").value = student.employment_status ?? "";

  // 이수현황 + 프로그램 + 시험결과를 함께 조회
  const [{ data: enrollments }, { data: attempts }] = await Promise.all([
    supabaseClient.from("enrollments").select("id, program_id, completed_hours, status, programs(id, name, required_hours, has_exam, equivalent_group)").eq("student_id", student.id),
...
function openEditEnroll(enrollmentId) {
  ...
  document.getElementById("editStatus").value = row.status ?? "미이수";
...
async function saveEnrollment() {
  ...
  const { error } = await supabaseClient
    .from("enrollments")
    .update({ status, completed_hours: hours })
...
async function saveEmployment() {
  const employment_status = document.getElementById("employmentBox").value;
  const { error } = await supabaseClient.from("students").update({ employment_status }).eq("id", currentStudent.id);
  if (error) alert("취업여부 저장 실패: " + error.message);
  else alert("취업여부가 저장되었습니다.");
}
