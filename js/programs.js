...
(async function init() {
  ...
  if (!auth) return;
  renderLayout("programs", auth.admin.name, auth.admin.department);
...
async function loadPrograms() {
  const [{ data: programs, error }, { data: enrollments }] = await Promise.all([
    ...
    fetchAllRows("enrollments", "program_id, status"),
  ]);
  if (error) {
    document.getElementById("programsBody").innerHTML = `<tr><td colspan="5" class="text-danger text-center py-3">불러오기 실패: ${error.message}</td></tr>`;
    return;
  }
...
function renderProgramsTable() {
  ...
  if (!programsCache.length) {
    tbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted py-3">등록된 프로그램이 없습니다.</td></tr>`;
    return;
  }
  tbody.innerHTML = programsCache
    .map((p) => {
      ...
      return `<tr>
        <td>${p.name}</td>
        <td>${p.equivalent_group ? `<span class="badge-status" style="background:#EEF2FB; color:var(--navy);">${p.equivalent_group}</span>` : "-"}</td>
...
function openEditProgram(id) {
  ...
  form.elements["equivalent_group"].value = p.equivalent_group ?? "";
...
function downloadProgramTemplate() {
  ...
  const guideWs = XLSX.utils.aoa_to_sheet([
    ...
    [""],
    ["- 동일과목그룹: 공통교과/비교과/글산과 버전처럼 사실상 같은 과목인 프로그램들에"],
    ["  똑같은 그룹명을 입력하면, 한 학생 기준으로 그 중 하나만(패스가 아닌 트랙) 이수시간과"],
    ["  필요시간 계산에 반영되어 중복 계산되지 않습니다. 해당 없으면 비워두세요."],
  ]);
...
const PROGRAM_EXCEL_HEADER_MAP = {
  ...
  "담당자": "manager_name",
  "동일과목그룹": "equivalent_group",
  "동일과목 그룹": "equivalent_group",
};
