...
// -----------------------------------------------------------
async function fetchAllRows(tableName, selectStr, filterFn, orderColumn) {
  const pageSize = 1000;
  let from = 0;
  let allRows = [];
  const orderCol = orderColumn || "id"; // 기본키가 id가 아닌 테이블은 orderColumn을 직접 지정해서 호출하세요.

  while (true) {
    // range 페이지네이션은 정렬 기준이 없으면 페이지 간 순서가 흔들려 데이터가
    // 누락되거나 중복될 수 있으므로, 항상 정렬 기준 컬럼으로 정렬해서 안정적으로 가져옵니다.
    let query = supabaseClient.from(tableName).select(selectStr).order(orderCol, { ascending: true }).range(from, from + pageSize - 1);
...
//
// 반환값: { totalRequired, totalCompleted, overallStatus, anyGroupCompleted }
// anyGroupCompleted: 동일과목 그룹(또는 단독 프로그램) 중 "이수"로 확정된 과목이 하나라도 있는지
// -----------------------------------------------------------
function computeGroupedCompletion(programs, studentEnrollments) {
  ...
  Object.values(groups).forEach((g) => {
    ...
    if (nonPassRows.length > 0) {
      ...
      if (rep.status === "이수") anyGroupCompleted = true;
      if (rep.status !== "이수") allGroupsDone = false;
      if (Number(rep.completed_hours || 0) > 0 || rep.status === "진행중") anyProgress = true;
    } else {
      // 그룹 내 모든 트랙이 "패스"(또는 이수현황 레코드 자체가 없음)
      // -> 패스는 이수시간에 반영하지 않습니다. (필요시간에는 그대로 포함되어
      //    분모는 항상 고정되지만, 분자는 실제 이수한 시간만 인정됩니다)
      allGroupsDone = false;
    }
  });
  ...
  return { totalRequired, totalCompleted, overallStatus, anyGroupCompleted };
}

// -----------------------------------------------------------
// 자동 수료구분 판정
// -----------------------------------------------------------
// 1) 총 이수시간 120시간 이상 -> 정규수료
// 2) 120시간 미만이고, 과목을 1개 이상 "이수"함 -> 단일수료
// 3) 120시간 미만이고, 이수한 과목이 하나도 없음 -> 미수료
// ※ "조기취업"은 관리자가 직접 지정하는 예외값이라 자동 계산 대상이 아닙니다.
//   (현재 값이 이미 "조기취업"이면 자동 계산으로 덮어쓰지 않습니다 — 호출하는 쪽에서 처리)
// -----------------------------------------------------------
function determineAutoCompletionType(totalCompleted, anyGroupCompleted) {
  const REQUIRED_TOTAL_HOURS = 120;
  if (totalCompleted >= REQUIRED_TOTAL_HOURS) return "정규수료";
  if (anyGroupCompleted) return "단일수료";
  return "미수료";
}

// -----------------------------------------------------------
// 우수학생 평가 등급 기준 및 등급별 장려금 (공용 상수)
// -----------------------------------------------------------
const GRADE_CRITERIA = [
  { grade: "S", minScore: 90, maxScore: 100, amount: 500000 },
  { grade: "A", minScore: 80, maxScore: 89, amount: 300000 },
  { grade: "B", minScore: 70, maxScore: 79, amount: 200000 },
];
const GRADE_AMOUNTS = { S: 500000, A: 300000, B: 200000 };
...
// 학과 묶음(department_groups)을 반영해, 해당 학과가 속한 "정원 관리 키"를 구합니다.
// (묶음이 지정되어 있으면 묶음 이름을, 없으면 학과명 그대로를 키로 사용)
function resolveQuotaGroupKey(department, departmentGroupMap) {
  return departmentGroupMap[department] || department;
}
