...
async function loadStudents() {
  ...
  allStudentsCache = (students || [])
    ...
    const { totalRequired, totalCompleted, overallStatus } = computeGroupedCompletion(programs, rows);
...
function renderStudentsTable(list) {
  ...
  if (!list.length) {
    tbody.innerHTML = `<tr><td colspan="12" class="text-center text-muted py-3">등록된 학생이 없습니다.</td></tr>`;
    return;
  }
  tbody.innerHTML = list
    .map(
      (s) => `<tr>
        ...
        <td>
          <select class="form-select form-select-sm" style="min-width:110px;" onchange="updateCompletionType('${s.id}', this.value)">
...
// 수료구분 변경 (관리자가 직접 지정)
// ---------------------------------------------------------
async function updateCompletionType(studentId, completionType) {
    ...
    const rowsToUpsert = Array.from(enrollmentPatchMap.values())
      .map(({ student_no, program_id, completed_hours, attendance_status }) => {
        ...
        if (completed_hours !== undefined) {
          patch.completed_hours = completed_hours;
          // 참석상태 컬럼을 채우지 않았어도, 이수시간이 입력되어 있다면
          // "참석"한 것으로 간주해 기본값을 자동으로 채웁니다.
          // (참석상태 컬럼을 별도로 채웠다면 아래에서 그 값으로 덮어씁니다.)
          patch.attendance_status = completed_hours > 0 ? "참석" : "결석";
        }
