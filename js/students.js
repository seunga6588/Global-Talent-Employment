  ...
  tbody.innerHTML = list
    .map(
      (s) => `<tr>
        ...
        <td>${s.employment_status ? `<span class="badge-status badge-emp-${s.employment_status}">${s.employment_status}</span>` : "-"}</td>
        <td>${s.kwork_id ? "등록" : "-"}</td>
...
function downloadTemplate() {
  ...
  const guideWs = XLSX.utils.aoa_to_sheet([
    ...
    ["- 취업여부 컬럼에는 취업 / 미취업 / 취업예정 중 하나만 입력하세요."],
