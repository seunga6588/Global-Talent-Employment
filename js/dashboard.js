...
(async function init() {
  ...
  if (!auth) return;
  renderLayout("dashboard", auth.admin.name, auth.admin.department);
...
async function loadDashboard() {
  ...
  renderDepartmentChart(students, enrollments, programs);
...
function renderStatCards(students, programs, enrollments, attempts) {
  ...
  students.forEach((s) => {
    ...
    const { totalCompleted, overallStatus } = computeGroupedCompletion(programs, rows);
    totalHoursSum += totalCompleted;
    if (overallStatus === "이수") completed++;
    else if (overallStatus === "진행중") inProgress++;
    else notStarted++;
  });

  const avgHours = totalStudents ? (totalHoursSum / totalStudents).toFixed(1) : "0.0";
  ...
  document.getElementById("statTotalHours").textContent = totalHoursSum + "시간";
...
function renderDepartmentChart(students, enrollments, programs) {
  // 학과별로 "전체 이수 완료(동일과목 그룹 반영)" 비율을 계산합니다.
  const byStudentRows = {};
  enrollments.forEach((e) => {
  ...
  students.forEach((s) => {
    ...
    const { overallStatus } = computeGroupedCompletion(programs, rows);
    if (overallStatus === "이수") deptStats[dept].completed += 1;
  });
